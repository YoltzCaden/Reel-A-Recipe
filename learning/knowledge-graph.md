...
- introduced: 2026-08-05
- last-reviewed: 2026-08-11
- evidence: passed the `input` element where `input.value` was needed, correctly said "it won't show the value, that's all i know" rather than guessing, then ran it and saw `[object HTMLInputElement]` land in the list. Met the rule that `[object Something]` in the UI means an object was passed where text was expected. 2026-08-11: transferred that rule unaided to a brand-new context weeks later — diagnosed a real `[object Object]` bug in `loadRecipes()`/`loadPantry()` (row objects from a real `pool.query()` result being passed whole to `addItem` instead of `.name`), correctly explaining the mechanism ("an object was parsed when text was expected") before being told anything, then located and fixed both occurrences themselves
